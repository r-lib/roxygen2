...
* New `@R6method Class$method` tag allows you to document R6 methods that can't be discovered by introspection, such as methods added via `$set()` or fields that become methods at runtime (#931, #991). Use it in a separate roxygen block outside the R6 class definition.
