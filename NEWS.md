...
* Added initial support for S7 classes, generics, and methods (#1484). S7 classes and generics are documented like regular functions. Use `@prop` to document additional properties that are not constructor parameters. S7 methods registered with `method(generic, class) <- fn` are detected automatically and generate usage with `## S7 method for class <ClassName>`.
