...
* New tag `@rawRd` allows you to insert raw (unescaped) Rd code. `@evalRd()` is 
  ...
  output. Similarly, `@rawNamespace` allows you to insert literal text into 
  the `NAMESPACE`: this can be useful for conditional imports (#385). 
...
* Roxygen2 now parses the source code files in the order specified in the
  ...
  several .R files, as often happens when working with S4 classes, generic 
  functions and methods (#323, #324).

## Minor features and bug fixes

* The contents of documented functions are now also parsed for tags. This 
  allows,e.g., documenting a parameter's type close to where this type is 
...
* Data objects get a simpler default `@format` that describes only the
  object's class and its dimensions.  The former default, generated by
  generated by `str()`, didn't usually produce useful output and was quite
  slow. The new S3 generic `default_data_frame()` generates the format and
  can be overridden to generate a custom format (#410, @krlmlr).
...
* `@section`s with identical titles are now merged together, just like 
  `@description` and `@details`.  This is especially useful in conjunction with 
