...
had_roclet <- function(package.dir) {
  ...
  process <- function(partita) {
    ...
    for (partitum in partita) {
      ...
      rd <- c(rd, 
        ...
        process_tag(partitum, 'aliases', function(tag, param) {
          process.split('alias', param)
        }),
        rd_tag('title', title),
        process.usage(partitum),
        process.arguments(partitum),
        process_tag(partitum, 'docType'),
        process_tag(partitum, 'description', process.description),
        process_tag(partitum, 'note'),
        process_tag(partitum, 'author'),
        process_tag(partitum, 'seealso'),
        process_tag(partitum, "references"),
        process_tag(partitum, 'concept'),
        process_tag(partitum, 'return', function(tag, param) {
          rd_tag('value', param)
        }),
        process_tag(partitum, 'keywords', function(tag, param, all, rd) {
          process.split('keyword', param)
        }),
        process_tag(partitum, 'TODO', process.todo),
  ...
  process_tag <- function(partitum, tag, f = rd_tag) {
    params <- partitum[[tag]]
  ...
  output <- function(results) { 
    subdir <- normalizePath(file.path(package.dir, "man"))
    ...
    paths <- file.path(subdir, names(results))
