...
#' 
#' Supports a limited subset of Markdown syntax in Roxygen comments: \itemize{ 
#' \item itemized lists (lines starting with a star folowed by a blank,
#' subsequent lines for the same item starting with 4 spaces) \item enumerated
#' lists (first item starting with 1 followed by a period, subsequent items
#' starting with numbers+period or star)\item \code{code}-formatting with
#' backticks (\code{`<code>`}) \item strong/bold (\code{**<bold>**}) \item
#' italic/emphasized text (\code{*<emph>*}) \item links to other function docs 
#' (\code{![<function>](<package>)}) \item inline math (\code{$<math>$}) \item
#' displayed equations (\code{$$<math>$$})}. 
