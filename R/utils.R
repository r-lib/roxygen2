...
with_srcref <- function(src, env = caller_env(), file = "file.R") {
  path <- file.path(tempfile("sourced"), file)
  dir.create(dirname(path))
  on.exit(unlink(dirname(path), recursive = TRUE))

  write_lines(src, file)
  exprs <- parse(file, keep.source = TRUE)

  last <- withVisible(NULL)

  for (i in seq_along(exprs)) {
    roxy_meta_local(current_srcref = attr(exprs, "srcref")[[i]])
    last <- withVisible(eval_bare(exprs[[i]], env))
  }
