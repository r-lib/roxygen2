...
Thanks for using roxygen2. Before filing an issue, there are a few
...
roxygen2 issues can be tricky to create a minimal reprex for. There are
two general techniques that can help:

- If you know the source of the problem, you can often recreate it using
  a
...
- Issues that involve multiple files or that don’t give useful error
  messages are harder to track down. In these cases, sometimes your best
  option is to create a copy of your package, and progressively delete
  files until the problem goes away. Once you’ve done that, sometimes
  you’ll be able to reduce the problem further to a call to
  [`roc_proc_text()`](https://roxygen2.r-lib.org/dev/reference/roc_proc_text.md)
  as above. If not, you’ll need to make your minimal package available
  somewhere on the internet (preferably github) and link to it in the
  issue. The more you can do to make the package as small as possible,
  the less time it’ll take me to find the bug, and the more time I’ll
  have to work on it.
...
requests](https://github.com/r-lib/roxygen2/issues) to make sure the bug
...
to be very bursty. Nothing happens for a long time, until a sufficient
quantity of issues accumulates, and then there’s a burst of intense
activity as we focus our efforts. That makes development more efficient
because it avoids expensive context switching between problems. This
process makes a good reprex particularly important because it might be
multiple months between your initial report and when we start working on
it. If you can’t reproduce the bug, we can’t fix it!
