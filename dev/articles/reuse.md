...
You can use `` `Rd ` `` to insert code that is evaluated at
documentation render time (i.e. when the user views the help page)
rather than at build time. For example, `` `Rd myFun()` `` will generate
`\Sexpr[stage=render,results=rd]{myFun()}` in the Rd file. This is
useful for documentation that needs to reflect the current state of the
system, not the state when the package was built.
