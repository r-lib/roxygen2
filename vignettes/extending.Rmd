...
The roxygen2 package is extensible.
There are two primary ways to extend roxygen2:

-   Add a new tag that generates a new top-level section in `.Rd` file to repeat yourself less when documenting your package. (See `vignette("reuse")` for other methods to repeat yourself less)

-   Add a new roclet to take advantage of `roxygenize()` to compute anything and produce any artefact you want.

This vignette will introduce you to the basic workflow of `roxygenize()`, the key data structures in roxygen2, and then show you how to use these two extension points.
...
Before we talk about extending roxygen2, we need to first discuss how its workhorse function, `roxygenize()`, works.
Here is what `roxygenize()` does, step by step:
...
- Finds the roclets (see `roclet()`) from its `roclets` argument or the "roclets" option (`load_options()`). Rd roclet, NAMESPACE roclet, your roclet...

- Runs the different methods of all those roclets, in order and independently: clean,preprocess, process, output. In you think of the Rd roclet, its process methods digests information from the tag parsing, combines inherits, etc. to create the content of each documentation topic, whereas its output method writes the topics to disk.

The workflow of `roxygenize()` features the two main extension points:

- you can define a tag in a package that you tell roxygen2 to load, and it will be used.
- you can define a new roclet that does anything you like, and it will be used.
...
Also note that other parsers are documented in `tag_markdown()`'s manual page.
...
We called the custom section "tip" just like our tag, but we needn't have done that.
...
### Exporting or registering methods

Now that the three methods are created, we still need to make them available to `roxygenize()`.

If the package the tag is defined in _imports_ roxygen2, we can simply export methods using `@export`.

If the package the tag is defined it _suggests_ roxygen2, we need to register the methods at load time. This is the approach used in [ggplot2](https://github.com/tidyverse/ggplot2/pull/6437/changes) for its `@aesthetics` tag.

### Registering tags from another package

To use your new tags in another package, you must tell roxygen2 to load that package in order to find the new tags. 
For example, if you created some new tags in {packageFoo}, and would like to use these tags in your documentation for {packageBar}, append this like to the `DESCRIPTION` of {packageBar}:
...
[^vignette]: The no-longer recommended `vignette_roclet()` only needs the path to the package source as input, it does not use information from the tag parsing step.
...
We do not need to define a format method for an Rd section because our tag won't be used to produce Rd sections.[^tag]
...
Or you could register it as a [custom dependency](https://r-pkgs.org/description.html#sec-description-custom-fields).
...
Hopefully, this vignette was useful enough to help you get started, and if you have problems, please [file an issue](https://github.com/r-lib/roxygen2/issues/new)!