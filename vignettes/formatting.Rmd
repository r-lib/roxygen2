...
* `\code{\link{function}}`: function in this pacakage
...
You can use standard LaTeX (with no extensions) with:
...
1. A letter for each column giving the alignment: `l` = left, `r` = right,
   `c` = centre.

2. The contents of the table with columns should be by `\tab` and rows by `\cr`.
