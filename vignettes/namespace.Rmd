...
-   If the method is particularly complex or has many arguments that the generic does not, you can document it in its own file. In this case, just document it as if its a function.
-   You can use `@rdname` to document it with other methds for the generic. This is a good option if it's your generic, and you're providing a bunch of methods for different classes.
...
-   **Methods**: you only need to `@export` a method, if the generic lives in another package.
    Unlike S4, you must document S4 methods.
