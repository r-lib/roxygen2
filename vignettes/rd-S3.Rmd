...
Sometimes you will want to write a method for a generic in a suggested (not imported) package.
In this case, `@export` will not work because it assumes the generic is available in your `NAMESPACE`.
Instead, use `@exportS3Method pkg::generic` which uses delayed method registration, meaning the method will only be registered when the suggested package is loaded.
...
(roxygen2 does automatically handle the `all.equal` case for you, but this illustrates the basic problem and how to fix it.)
