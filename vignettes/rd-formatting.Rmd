...
The interpretation from source all the way to the final Rd documentation goes through three steps.
For example:
...
2. is then run through roxygen's markdown engine
...
To insert entire blocks of code, enclose them in ` ```{r} ` to start them, just like in knitr documents.
...
1. is run through knitr to evaluate R, yielding:
    ...
    ```{r code = roxygen2:::markdown_pass1(simple_fenced)}
    ```
    
2. is then run through roxygen's markdown engine, yielding:
    
    ```{r code = roxygen2:::markdown(simple_fenced)}
    ```
...
Every inline or fenced code chunk is evaluated separately as if they were separate R markdown document.
...
This causes some limitations and differences to keep in mind:
  ...
  * Inline and fenced code blocks from the same _roxgen block_ share the same evaluation environment. 
    ...
    ```r
    ```{r code = brio::read_lines("../tests/testthat/roxygen-block-md-code-envs.R")}
    ```
    ```
  ...
  * Knitr calls the appropriate `print()` or (if available) `knitr::knit_print()` methods on the result.
    The resulting markdown from some of these methods may not be supported by roxygens subset of markdown. 
    You can use `print.default()` or even `cat()` to return compliant markdown.
