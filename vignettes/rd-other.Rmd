...
S7 **methods** are registered with `method(generic, class) <- fn`. For straightforward implementations, you don't need to document the method separately. You may want to do so, however, if it has a particularly complicated implementation or has many additional arguments that the generic does not.
In that case, give it its own roxygen block:
...
This generates a usage line like `## S7 method for class <Range>`.
For multi-dispatch methods, all dispatch classes are shown.
...
-   To turn off the special handling of R6 classes and go back to the roxygen2 6.x.x behavior, use `Config/roxygen2/r6: FALSE` in `DESCRIPTION`.
