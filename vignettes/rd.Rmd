...
### Including an (R) markdown file

Starting from roxygen 7.0.0, you can use the `@includeRmd` tag to include an R markdown or plain markdown file in a manual page. You can include the same file in multiple manual pages, and you can also include these files in `README.Rmd` and in vignettes. See more about `@includeRmd` below.
...
## Including external (R) Markdown documents

The `@includeRmd file-name` tag includes an external (R) Markdown document in a manual page. `file-name` is a file name relative to the root directory of the package.
...
*   Other headings (level 2 and so on) create subsections with the section they appear in.

Note that all content of the Rmd file will go either in the details section or new top level sections. It is currently not possible to document function arguments, return values, etc. in external Rmd documents.

### Links

The included Rmd file can have roxygen markdown style links to other help topics. E.g. `[roxygen2::roxygenize()]` will link to the manual page of the `roxygenize` function in roxygen2. See the "Rd formatting" vignette for details.

### Caching and figures

`@includeRmd` tries to set up knits to support caching in the Rmd file. It sets the cache path to the default knitr cache path of the included Rmd file, so if you do not change the cache path within the Rmd itself, then everything should work out of the box.

`@includeRmd` also sets the path of the figures (`fig.path`) to the default figure path of the included Rmd.

### Sharing text between vignettes and the manual

`@includeRmd` helps avoiding repetition, as you can use the same (R) markdown document in the manual and also in the `README.md` file or in vignettes. One way to include an Rmd file in another one is to use child documents:
...
If the Rmd file has links to other help topics, then some care is needed here, as these links do not work in Rmd files, such as a `README.md` file. A workaround is to specify external HTML links for them. These external locations will _not_ be used for `@includeRmd`, which always links them to the help topics in the manual. Example:
