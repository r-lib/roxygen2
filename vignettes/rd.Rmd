...
Use `@describeIn` to surface the relationships between one or more "source" functions, and the destination file you want them documented in.
If you add `@describeIn <destination> <description>` to some source function, it will be documented inside the `<destination>` file (usually the function name).
...
Each subsections contains a bulleted list including your `<description>`.
...
1. If your new method extends a generic in the `<destination>`, it will be listed in a subsection titled "**Methods extending `<destination>` generic (by class)**".
    The subsection will include all methods for your new generic, labelled by the class they cover.

    ```{r, code = brio::read_lines("../tests/testthat/roxygen-block-describe-in-method-in-gen.R")}
    ```
    ...
    The method `print.qux()` will be labelled as a normal function, because it extends an unrelated generic `print()`.
    The `zap_helper()` function will, likewise, be displayed as a normal function.

1. If your new method extends another generic, for a class constructed in the destination, it will be listed in a subsection titled "**Methods extending `<destination>` class (by generic)**"
    It will be labelled by the generic which the method extends.
    
    ```{r, code = brio::read_lines("../tests/testthat/roxygen-block-describe-in-method-in-const.R")}
    ```
    
    The resulting output describes `print.foo()` (a method) and `is_foo()` (a normal function) under different headings, pointing users to the different semantics for both. 
    `format.bar()` will also be presented as a normal method, because it does not cover the class in question.
