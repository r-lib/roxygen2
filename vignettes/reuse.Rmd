...
`@inheritParams` (like all `@inherits` functions) works recursively, so if `g` inherits parameters from `h`, then `f` can also inherit those parameters from `g`. However, this technique is best used sparingly: it's very easy to create complex dependency webs that are hard to reason about. If you want to be more explicit, you should consider writing helper functions and using inline R code, as described below.
...
You can use `` `Rd ` `` to insert code that is evaluated at documentation render time (i.e. when the user views the help page) rather than at build time.
For example, `` `Rd myFun()` `` will generate `\Sexpr[stage=render,results=rd]{myFun()}` in the Rd file.
This is useful for documentation that needs to reflect the current state of the system, not the state when the package was built.
